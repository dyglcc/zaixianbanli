...
public final class Dispatcher {
    ...
    public synchronized void cancel(Object tag) {
        for (Call.AsyncCall call : readyCalls) {
            if (Util.equal(tag, call.tag())) {
                call.cancel();
            }
        }

        for (Call.AsyncCall call : runningCalls) {
            if (Util.equal(tag, call.tag())) {
    ...
    private void promoteCalls() {
        try {
            ...
            for (Iterator<Call.AsyncCall> i = readyCalls.iterator(); i.hasNext(); ) {
    ...
    private int runningCallsForHost(Call.AsyncCall call) {
        int result = 0;
        for (Call.AsyncCall c : runningCalls) {
